...
This document describes the structure of the tidy data frame that be be either:
- downloaded - by loading the act_subj_avg.txt text file in R using read.table
- generated  - Executing the run_analysis.R script generates a data frame named actSubjAvg

The run_analysis.R script us explain in the README.md file
...
### Variables
 ...
 Columns 3-88 contain the mean over all measurements for the given activity (column 1) and subject (column 2)
 Only variables which measures mean and standard deviation are included:
...
79. mean(fBodyBodyGyroJerkMag-std())           - the mean of fBodyBodyGyroJerkMag-std()           over all measurements for the given activity (column 1) and subject (column 2)         
...
80. mean(angle(tBodyAccMean,gravity))          - the mean of angle(tBodyAccMean,gravity)          over all measurements for the given activity (column 1) and subject (column 2)         
